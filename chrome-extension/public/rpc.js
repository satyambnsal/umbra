...
console.log('RPC Script injected');
...
var BROADCAST_CHANNEL_ID = 'umbra';
var callUmbraAsync = ({ method, payload }) => {
...
var debouncedCall = p_debounce_default(callUmbraAsync, 300);

// src/inject/rpc/provider.ts
var _events = mitt_default();
window.addEventListener('umbra_event', event => {
...
// src/inject/rpc.ts
var init = () => {
  window.umbra = provider;
  const announceProvider = () => {
    window.dispatchEvent(
      new CustomEvent('umbra:announceProvider', {
        detail: Object.freeze({ info, provider }),
      }),
    );
  };
  window.addEventListener('umbra:requestProvider', () => {
    announceProvider();
  });
  announceProvider();
  console.info('[Umbra] RPC has been initialized.');
};
init();
