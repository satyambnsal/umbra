...
import { RestartWalletAlert } from '@src/components/restart-wallet-alert';
import { WizardLayout } from '@src/components/wizard-layout';
...
export const UnlockWalletView = ({
  ...
  <WizardLayout
    ...
    <RestartWalletAlert open={restartAlertVisible} setOpen={setRestartAlertVisible} />
    <div className="w-full flex flex-col flex-1 items-center gap-12">
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-1 flex-col gap-2 w-full" id="unlockWalletForm">
        <label htmlFor="spendingPassword" className={clsx('label cursor-pointer')}>
          Spending Password
        </label>
        <label className="input flex items-center gap-2 w-full">
          ...
          />
          <button type="button" className="btn btn-link -mr-4" onClick={togglePassword}>
